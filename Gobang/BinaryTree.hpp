...
public:
	...
	using iterator_types::iterator_category;
	using iterator_types::value_type;
	using iterator_types::pointer;
	using iterator_types::reference;
	using iterator_types::NodePtr;
	using iterator_types::self;
	using iterator_types::difference_type;
	using iterator_types::size_type;
...
protected:
	...
	void _Transplant(NodePtr original, NodePtr newchild)
	{
		...
		original->_parent = nullptr;  //the situation that newchild is root isnt considered
		if (newchild->_parent != nullptr) {
			if (newchild == newchild->_parent->_left)
				newchild->_parent->_left = nullptr;
			else if (newchild == newchild->_parent->_right)
				newchild->_parent->_right = nullptr;
		}

		if (parent->_parent == nullptr)  //original is root
			_root = newchild;
